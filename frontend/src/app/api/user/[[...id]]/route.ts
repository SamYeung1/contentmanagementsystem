...
export async function GET(req: NextRequest, ctx: RouteContext<'/api/user/[[...id]]'>) {
  ...
  const orderBy = searchParams.has('orderBy') ? JSON.parse(searchParams.get('orderBy')!) : { key: 'id', direction: 'ASC' } ;
  const userSession = await getSession();
  const { id } = await ctx.params;
  if (id) {
    return Response.json(await get(id[0], userSession));
  } else {
    return Response.json(await list({ page: page, orderBy: orderBy as Direction }, userSession));
  }
